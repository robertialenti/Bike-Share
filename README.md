...
In order to generate aggregate statistics by station, it is important to have a reliable station identifier. The ride-level data from Bixi provides two potentially useful identifiers: $\text{Station Name}$ and $\text{Station Code}$. However, $\text{Station Name}$ is susceptible to changes in spelling, even within a given year, while $\text{Station Code}$ is found to change over time, even when referring to the same station.
...
<img src="https://github.com/robertialenti/Bixi/raw/main/figures/did_combined.png" width="425" height="250">
